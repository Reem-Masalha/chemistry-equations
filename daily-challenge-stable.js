...
const adminReplay=()=>sessionStorage.getItem('chemistryAdminSession')==='1'&&new URLSearchParams(location.search).has('adminReplay');
function mount(){
 ...
 const reviewHTML=()=>qs.map((q,i)=>{const a=s.answers[i]||T('No answer','لا توجد إجابة','אין תשובה');const ok=s.status[i]===1;return `<article class="daily-review-item"><b>${i+1}. ${q[0]}</b><div>${T('Your answer','إجابتك','התשובה שלך')}: <span>${a}</span></div><div>${T('Correct answer','الإجابة الصحيحة','התשובה הנכונה')}: <span>${q[1]}</span></div><div>${ok?'✓ '+T('Correct','صحيح','נכון'):'✕ '+T('Incorrect','غير صحيح','לא נכון')}</div><p>${q[2]}</p></article>`}).join('');
 const showReview=()=>{result.innerHTML=`<h3>${T('Your result','نتيجتك','התוצאה שלך')}: ${s.score}/5</h3><button id="dsvReview" class="primary" type="button">${T('Review daily quiz answers','مراجعة إجابات الاختبار اليومي','סקירת תשובות החידון היומי')}</button><div id="dsvReviewList" class="daily-review" hidden>${reviewHTML()}</div>`;result.hidden=false;const rb=result.querySelector('#dsvReview'),list=result.querySelector('#dsvReviewList');rb.addEventListener('click',()=>{list.hidden=false;rb.hidden=true})};
 const finish=()=>{clearInterval(interval);interval=null;if(!s.complete){s.complete=true;s.started=false;s.index=5;save()}controls(false);next.hidden=true;start.hidden=true;badge.textContent='✓ '+T('Completed today','أكملت تحدي اليوم','הושלם היום');renderProgress();eq.textContent=T('Challenge complete!','اكتمل التحدي!','האתגר הושלם!');score.textContent=String(s.score);showReview()};
 const render=()=>{clearInterval(interval);interval=null;feedback.textContent='';hintBox.hidden=true;if(s.complete||s.index>=5){finish();return}result.hidden=true;const q=qs[s.index];eq.textContent=q[0];badge.textContent='🔥 '+T('Question '+(s.index+1)+' of 5','السؤال '+(s.index+1)+' من 5','שאלה '+(s.index+1)+' מתוך 5');score.textContent=String(s.score);renderProgress();input.value=s.answers[s.index]||'';const recorded=s.status[s.index]>0;start.hidden=s.started;controls(s.started&&!recorded);next.hidden=!s.started;note.textContent=recorded?T('Answer submitted. Continue to the next question.','تم إرسال الإجابة. انتقل إلى السؤال التالي.','התשובה נשלחה. המשיכו לשאלה הבאה.'):T('Check gives feedback only. Submit or Next question records the answer and moves on.','التحقق يعطي ملاحظات فقط. الإرسال أو السؤال التالي يسجل الإجابة وينتقل للسؤال التالي.','בדיקה נותנת משוב בלבד. שליחה או השאלה הבאה רושמות את התשובה וממשיכות לשאלה הבאה.');if(s.started){interval=setInterval(tick,500);tick()}};
 ...
 const bind=(el,fn)=>el.addEventListener('click',e=>{e.preventDefault();e.stopImmediatePropagation();fn()},true);
 bind(start,startChallenge);bind(check,checkAnswer);bind(submit,finalize);bind(next,finalize);bind(hint,showHint);
 if(adminReplay()){s.started=true;s.endAt=Date.now()+120000;save()}
 render();
}
