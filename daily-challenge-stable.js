...
function mount(){
 ...
 const anchor=main.querySelector('#course-map')||main.firstElementChild; if(anchor)anchor.insertAdjacentElement('afterend',section);else main.prepend(section);
 ...
 function evaluate(finalSubmission){if(!started||complete||status[index]>0)return;const value=input.value.trim();if(!value){feedback.textContent='⚠️ '+T('Enter an answer first.','اكتب إجابة أولًا.','הקלידו תשובה קודם.');return}answers[index]=value;const correct=normalize(value)===normalize(qs[index][1]);if(correct){status[index]=1;score++;feedback.textContent='✓ '+T('Correct! Great job.','صحيح! أحسنت.','נכון! עבודה מצוינת.');note.textContent=T('Correct. Continue when ready.','إجابة صحيحة. انتقل عندما تكون مستعدًا.','נכון. המשיכו כשתהיו מוכנים.')}else if(finalSubmission){status[index]=2;feedback.textContent='✕ '+T('Answer submitted. No point added.','تم إرسال الإجابة. لم تُضف نقطة.','התשובה נשלחה. לא נוספה נקודה.');note.textContent=T('Final answer recorded.','تم تسجيل الإجابة النهائية.','התשובה הסופית נרשمت.')}else{feedback.textContent='❌ '+T('Not quite. Edit your answer and check again, or submit it.','ليس تمامًا. عدّل إجابتك وتحقق مرة أخرى، أو أرسلها.','לא בדיוק. ערכו ובדקו שוב, או שלחו.');save();return}save();input.disabled=true;checkButton.disabled=true;submitButton.disabled=true;nextButton.hidden=false}
...
const style=document.createElement('style');style.id='daily-stable-style';style.textContent=`#daily-stable{margin:24px 0}.daily-stable-card{padding:24px;border:1px solid var(--line,#dce3ee);border-radius:20px;background:var(--surface,#fff);box-shadow:0 10px 28px rgba(25,43,76,.07)}.daily-stable-head{display:flex;justify-content:space-between;gap:16px;align-items:flex-start}.daily-stable-kicker{font-size:11px;letter-spacing:.12em;font-weight:800;color:var(--accent)}.daily-stable-head h2{margin:6px 0}.daily-stable-head p{margin:0;color:var(--muted)}.daily-stable-badge{padding:7px 10px;border-radius:999px;background:#edf1ff;font-size:12px;font-weight:800;white-space:nowrap}.daily-stable-progress{display:flex;gap:6px;margin:16px 0}.daily-stable-dot{height:8px;flex:1;border-radius:99px;background:#e4e9f1}.daily-stable-dot.done{background:var(--accent)}.daily-stable-dot.current{box-shadow:0 0 0 2px rgba(49,88,214,.2)}.daily-stable-meta{display:flex;justify-content:space-between;gap:12px;color:var(--muted);font-size:13px}.daily-stable-equation{direction:ltr;text-align:center;font-size:clamp(25px,4vw,40px);font-weight:900;padding:20px 10px;margin:18px 0;border:1px solid var(--line);border-radius:15px}.daily-stable-input{width:100%;box-sizing:border-box;padding:14px;border:1px solid var(--line);border-radius:11px;font-size:18px;text-align:center;direction:ltr;background:transparent;color:inherit}.daily-stable-actions{display:flex;flex-wrap:wrap;gap:9px;margin-top:10px}.daily-stable-actions>*{min-height:44px}.daily-stable-feedback{min-height:30px;margin-top:12px;font-weight:800}.daily-stable-note,.daily-stable-hint{margin-top:9px;padding:12px 14px;border:1px solid var(--line);border-radius:12px;background:var(--surface-2,#f7f9fc);color:var(--muted)}.daily-stable-result{margin-top:16px;padding:18px;border:1px solid var(--line);border-radius:15px}.daily-stable-score{font-size:38px;font-weight:900}@media(max-width:760px){.daily-stable-card{padding:17px}.daily-stable-head,.daily-stable-meta{flex-direction:column}.daily-stable-badge{align-self:flex-start}.daily-stable-actions>*{flex:1 1 145px}}`;document.head.appendChild(style);
