...
function mount(){
 ...
 const render=()=>{clearInterval(interval);interval=null;feedback.textContent='';hintBox.hidden=true;if(s.complete||s.index>=5){finish();return}result.hidden=true;const q=qs[s.index];eq.textContent=q[0];badge.textContent='🔥 '+T('Question '+(s.index+1)+' of 5','السؤال '+(s.index+1)+' من 5','שאלה '+(s.index+1)+' מתוך 5');score.textContent=String(s.score);renderProgress();input.value=s.answers[s.index]||'';const recorded=s.status[s.index]>0;start.hidden=s.started;controls(s.started&&!recorded);next.hidden=!recorded;note.textContent=recorded?T('Answer submitted. Continue to the next question.','تم إرسال الإجابة. انتقل إلى السؤال التالي.','התשובה נשלחה. המשיכו לשאלה הבאה.'):T('Check gives feedback only. Submit records the answer and moves on.','التحقق يعطي ملاحظات فقط. الإرسال يسجل الإجابة وينتقل للسؤال التالي.','בדיקה נותנת משוב בלבד. שליחה רושמת את התשובה וממשיכה לשאלה הבאה.');if(s.started){interval=setInterval(tick,500);tick()}};
 ...
 start.addEventListener('click',e=>{e.preventDefault();e.stopPropagation();startChallenge()});
 check.addEventListener('click',e=>{e.preventDefault();e.stopPropagation();checkAnswer()});
 submit.addEventListener('click',e=>{e.preventDefault();e.stopPropagation();finalize()});
 next.addEventListener('click',e=>{e.preventDefault();e.stopPropagation();finalize()});
 hint.addEventListener('click',e=>{e.preventDefault();e.stopPropagation();showHint()});
