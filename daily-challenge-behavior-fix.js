...
const BANK=[
['H₂ + O₂ → H₂O','2H₂ + O₂ → 2H₂O','Balance H first: put 2 before H₂O. Oxygen then matches.'],['Na + Cl₂ → NaCl','2Na + Cl₂ → 2NaCl','Cl₂ has two chlorine atoms, so use 2NaCl, then match sodium.'],['Mg + O₂ → MgO','2Mg + O₂ → 2MgO','O₂ has two oxygen atoms, so use 2MgO, then 2Mg.'],['N₂ + H₂ → NH₃','N₂ + 3H₂ → 2NH₃','Use 2NH₃ to match N₂, then use 3H₂ for hydrogen.'],['Fe + O₂ → Fe₂O₃','4Fe + 3O₂ → 2Fe₂O₃','Use 2Fe₂O₃ to make six oxygen atoms, then match iron and O₂.'],['Zn + HCl → ZnCl₂ + H₂','Zn + 2HCl → ZnCl₂ + H₂','Zn is already balanced. Use 2HCl to supply two chlorine atoms.'],['KClO₃ → KCl + O₂','2KClO₃ → 2KCl + 3O₂','Make six oxygen atoms with 2KClO₃, then use 3O₂.'],['Na₂O + H₂O → NaOH','Na₂O + H₂O → 2NaOH','There are two sodium atoms, so use 2NaOH.'],['C₃H₈ + O₂ → CO₂ + H₂O','C₃H₈ + 5O₂ → 3CO₂ + 4H₂O','For combustion, balance C first, H second, and O last.'],['NH₃ + O₂ → NO + H₂O','4NH₃ + 5O₂ → 4NO + 6H₂O','Balance H first, then finish oxygen.'],['FeS₂ + O₂ → Fe₂O₃ + SO₂','4FeS₂ + 11O₂ → 2Fe₂O₃ + 8SO₂','Balance Fe first, then S, and leave O until last.'],['Ca(OH)₂ + HCl → CaCl₂ + H₂O','Ca(OH)₂ + 2HCl → CaCl₂ + 2H₂O','Use 2HCl for Cl₂, then 2H₂O balances H and O.'],['Al + O₂ → Al₂O₃','4Al + 3O₂ → 2Al₂O₃','Make six O atoms with 2Al₂O₃, then match aluminum.'],['CO + O₂ → CO₂','2CO + O₂ → 2CO₂','Use 2CO₂ so carbon and oxygen balance.'],['P + O₂ → P₂O₅','4P + 5O₂ → 2P₂O₅','Use 2P₂O₅ for four phosphorus atoms, then 5O₂.'],['H₂ + Cl₂ → HCl','H₂ + Cl₂ → 2HCl','Both reactants contain two atoms, so use 2HCl.'],['Ag + S → Ag₂S','2Ag + S → Ag₂S','Ag₂S contains two Ag atoms, so use 2Ag.'],['CH₄ + O₂ → CO₂ + H₂O','CH₄ + 2O₂ → CO₂ + 2H₂O','Balance C first, H second, and O last.'],['C₂H₆ + O₂ → CO₂ + H₂O','2C₂H₆ + 7O₂ → 4CO₂ + 6H₂O','Balance C and H first, then oxygen.'],['CaCO₃ → CaO + CO₂','CaCO₃ → CaO + CO₂','All elements are already balanced, so all coefficients are 1.'],['Cu + O₂ → CuO','2Cu + O₂ → 2CuO','Use 2CuO to match O₂, then use 2Cu.'],['Cl₂ + NaBr → NaCl + Br₂','2NaBr + Cl₂ → 2NaCl + Br₂','Keep Br₂ together; use 2NaBr and 2NaCl.'],['H₂O₂ → H₂O + O₂','2H₂O₂ → 2H₂O + O₂','Use 2H₂O₂ so the remaining oxygen forms O₂.'],['SO₂ + O₂ → SO₃','2SO₂ + O₂ → 2SO₃','Use 2SO₃ so sulfur and oxygen balance.'],['NO + O₂ → NO₂','2NO + O₂ → 2NO₂','Use 2NO₂ so nitrogen is 2:2 and oxygen is 4:4.']
];
...
function keepNextVisible(){const root=document.getElementById('daily-stable');if(!root)return;const s=read();const next=root.querySelector('#dsvNext');if(next&&s.started&&!s.complete)next.hidden=false}
function attach(){
 ...
 let check=replaceButton('dsvCheck'),submit=replaceButton('dsvSubmit'),next=replaceButton('dsvNext');
 if(root.dataset.behaviorFixed){keepNextVisible();return}
...
function addReview(){
 ...
 const btn=document.createElement('button');btn.id='dsvReviewBtn';btn.className='primary';btn.type='button';btn.textContent='📋 '+T('Review daily quiz answers','مراجعة إجابات الاختبار اليومي','סקירת תשובות החידון היומי');result.appendChild(btn);
 btn.addEventListener('click',()=>{
   ...
   QUESTIONS().forEach((q,n)=>{
     ...
     const ok=status[n]===1;const title=document.createElement('div');title.textContent=(ok?'✓ ':'✕ ')+T('Question '+(n+1),'السؤال '+(n+1),'שאלה '+(n+1));title.style.fontWeight='900';
...
const run=()=>{attach();keepNextVisible();addReview()};
