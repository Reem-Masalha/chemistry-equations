(()=>{
  ...
  if(first)post('/api/track-visit',{visitorId:identity,path});
  if(user?.id){
    const heartbeat=()=>post('/api/track-event',{visitorId:identity,userId:String(user.id),eventType:'heartbeat',feature:null,metadata:{path}});
    heartbeat();
