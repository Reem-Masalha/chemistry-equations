(()=>{
  ...
  const get=()=>{const v=localStorage.getItem(KEY);return langs[v]?v:'en'};
  function apply(){
    const l=get();
    ...
    if(!body)return;
    body.classList.toggle('rtl',langs[l].dir==='rtl');
    document.querySelectorAll('[data-i18n]').forEach(e=>{const k=e.dataset.i18n;if(t[l][k]!=null)e.textContent=t[l][k]});
    document.querySelectorAll('[data-i18n-placeholder]').forEach(e=>{const k=e.dataset.i18nPlaceholder;if(t[l][k]!=null)e.placeholder=t[l][k]});
  ...
  function addSwitcher(){
    ...
    s.id='languageSwitcher';s.className='secondary';s.setAttribute('aria-label','Language');
