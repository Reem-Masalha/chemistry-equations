(() => {
  ...
  // This controller owns the canvas interaction. It runs in capture phase so the
  // older drawing handlers cannot also draw over the same stroke.
  let history = [];
  let future = [];
  let drawing = false;
  let erasing = false;
  ...
  const save = () => history.push(ctx.getImageData(0, 0, canvas.width, canvas.height));
  const restore = img => { ctx.clearRect(0,0,canvas.width,canvas.height); if (img) ctx.putImageData(img,0,0); };
  const startState = () => { history = []; future = []; save(); };
  startState();
  ...
  function begin(e) {
    ...
    drawing = true; last = pos(e); save(); future = [];
  }
  function move(e) {
    if (!drawing) return;
    e.stopImmediatePropagation(); e.preventDefault();
  ...
  function end(e) { if (!drawing) return; e.stopImmediatePropagation(); e.preventDefault(); drawing=false; last=null; }
  ...
  function button(id, text) {
    ...
    if (!b && erase?.parentElement) { b=document.createElement('button'); b.id=id; b.className='secondary'; b.type='button'; erase.parentElement.insertBefore(b, erase); }
    if (b) b.textContent=text;
    return b;
  }
  const undo = button('undoPad','Undo');
  const redo = button('redoPad','Redo');
  if (erase) erase.textContent='Eraser';
  ...
  redo?.addEventListener('click', e => {
    ...
    const img=future.pop(); history.push(img); restore(img);
  });
  clear?.addEventListener('click', e => {
    e.preventDefault(); e.stopImmediatePropagation();
    restore(null); history=[]; future=[]; save();
  ...
  erase?.addEventListener('click', e => {
    e.preventDefault(); e.stopImmediatePropagation();
    erasing=!erasing;
    erase.textContent=erasing?'Eraser ✓':'Eraser';
    erase.classList.toggle('primary',erasing); erase.classList.toggle('secondary',!erasing);
  });

  const toSub = s => String(s||'').replace(/([A-Z][a-z]?)(\d+)/g, '$1$2');
  const normalize = s => toSub(String(s||'').replace(/[\r\n]+/g,' ').replace(/[→⟶⇒➜=]/g,' -> ').replace(/\s*[-]+>\s*/g,' -> ').replace(/\s*\+\s*/g,' + ').replace(/\s+/g,' ').trim());
  const pretty = s => String(s||'').replace(/([A-Z][a-z]?)(\d+)/g,'$1<sub>$2</sub>').replace(/\s*->\s*/g,' → ');
  ...
  async function recognizeCloudflare(image) {
    ...
    const r=await fetch(base.replace(/\/$/,'')+'/recognize',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({image})});
  ...
  async function recognize() {
    ...
    if(edit&&correction){edit.value=text;correction.classList.remove('hidden');if(conf)conf.innerHTML=score===null?'No confidence score was returned. Please verify every symbol.':`Recognition confidence: <b>${score}%</b>. ${uncertain?'Please correct the equation before balancing.':'Please verify chemical capitalization and subscripts.'}`;}
  }
  if(use){use.onclick=e=>{e.preventDefault();e.stopImmediatePropagation();recognize();};}
  document.getElementById('balanceRecognized')?.addEventListener('click',()=>{
    const v=document.getElementById('recognizedEdit')?.value||'';
    const input=document.getElementById('equationInput'); if(input) input.value=v;
    if(typeof window.showBalance==='function') window.showBalance(v); else document.getElementById('balanceBtn')?.click();
  });
})();
