(() => {
  ...
  function move(e) { if(!drawing)return; e.preventDefault(); e.stopImmediatePropagation(); const p=pos(e); ctx.save(); ctx.globalCompositeOperation=erasing?'destination-out':'source-over'; ctx.strokeStyle='#142033'; ctx.lineWidth=erasing?28:4; ctx.lineCap='round'; ctx.lineJoin='round'; ctx.beginPath();ctx.moveTo(last.x,last.y);ctx.lineTo(p.x,p.y);ctx.stroke();ctx.restore();last=p; }
