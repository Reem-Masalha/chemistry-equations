(()=>{
  ...
  copyPad?.addEventListener('click', async e => { e.preventDefault(); e.stopImmediatePropagation(); try { const blob = await new Promise(resolve => canvas.toBlob(resolve, 'image/png')); if(!blob) throw new Error('Could not create image.'); if(navigator.clipboard?.write && typeof ClipboardItem !== 'undefined'){await navigator.clipboard.write([new ClipboardItem({'image/png':blob})]);if(status)status.textContent='Handwriting copied to clipboard.'}else throw new Error('Image clipboard is not supported by this browser.'); } catch(err){if(status)status.textContent=err.message||'Copy failed. Clipboard permission may be required.';} });
  async function pasteImageBlob(blob){if(!blob||!blob.type.startsWith('image/'))return false;const url=URL.createObjectURL(blob);try{const img=new Image();await new Promise((resolve,reject)=>{img.onload=resolve;img.onerror=reject;img.src=url});const scale=Math.min(canvas.width/img.naturalWidth,canvas.height/img.naturalHeight,1),w=img.naturalWidth*scale,h=img.naturalHeight*scale,x=(canvas.width-w)/2,y=(canvas.height-h)/2;ctx.save();ctx.globalCompositeOperation='source-over';ctx.drawImage(img,x,y,w,h);ctx.restore();commitState();if(status)status.textContent='Image pasted into handwriting canvas.';return true}finally{URL.revokeObjectURL(url)}}
