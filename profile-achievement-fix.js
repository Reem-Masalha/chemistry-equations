...
const isPreview=()=>{const b=document.getElementById('previewAchievements'),r=document.getElementById('realProfile');return !!(b&&r&&b.style.display==='none'&&r.style.display!=='none')};
const update=rows=>{
 ...
 if(next){const locked=cards.find(x=>x.classList.contains('locked'));if(locked){const title=(locked.querySelector('h3')?.textContent||'').replace(/[🔒✓]/g,'').trim();const progress=locked.querySelector('small')?.textContent||'';next.innerHTML='<div class="progress-row"><b>Next achievement: '+esc(title)+'</b><small class="empty-progress">'+esc(progress)+'</small></div>'}else next.innerHTML='<div class="progress-row"><b>🎉 All current achievements unlocked!</b></div>'}
 return true;
};
...
let tries=0;const wait=()=>{if(isPreview()){return}if(update([])){run();return}if(++tries>30){run();return}setTimeout(wait,100)};setTimeout(wait,120);
})();