  ...
  async fetch(request, env) {
    ...
    try {
      ...
      if (url.pathname === '/api/track-visit' && request.method === 'POST') {
        ...
        await env.DB.prepare('INSERT INTO visits (visitor_id,path,created_at) VALUES (?,?,?)')
          .bind(String(visitorId), String(path).slice(0, 200), new Date().toISOString()).run();
      ...
      if (url.pathname === '/api/create-account' && request.method === 'POST') {
        ...
        await env.DB.prepare('INSERT INTO users (id,name,username,password_hash,password_salt,created_at) VALUES (?,?,?,?,?,?)')
          .bind(id, String(name).trim(), u, hash, salt, new Date().toISOString()).run();
