...
async function pbkdf2Hex(password, salt, iterations = 150000) {
...
async function hashPassword(password) {
  ...
  const hash = await pbkdf2Hex(String(password), salt);
  return { salt, hash, stored: `pbkdf2_sha256$150000$${salt}$${hash}` };
}
...
async function verifyPassword(password, user) {
  ...
  // Canonical format used for all newly-created and repaired accounts.
  if (stored.startsWith("pbkdf2_sha256$")) {
    ...
    if (parts.length === 4) {
      ...
      if (Number.isInteger(iterations) && iterations >= 10000 && /^[a-f0-9]{64}$/i.test(expected)) {
  ...
  // Legacy SHA-256 records are retained only for compatibility. We never
  // overwrite them unless the password is actually verified or an admin
  // explicitly repairs the account.
  if (stored.toLowerCase().startsWith("sha256$") && !user.password_salt) {
    ...
    if (parts.length === 3) {
      ...
      if (/^[a-f0-9]{64}$/.test(expected)) {
        ...
        for (const candidate of candidates) if (safeEqual(candidate, expected)) return true;

        const hmacKey = await crypto.subtle.importKey(
          "raw", encoder.encode(salt), { name: "HMAC", hash: "SHA-256" }, false, ["sign"]
        );
        const hmac = hex(await crypto.subtle.sign("HMAC", hmacKey, encoder.encode(String(password))));
...
async function getUser(env, username) {
  return env.DB.prepare(`
    SELECT id,name,username,password_hash,password_salt,recovery_code_hash,recovery_code_expires_at,recovery_email
...
async function signIn(env, body) {
  ...
  // Upgrade a verified legacy password immediately to the single canonical format.
  if (!String(user.password_hash).startsWith("pbkdf2_sha256$")) {
    const p = await hashPassword(password);
    await env.DB.prepare(`UPDATE users SET password_hash = ?, password_salt = ? WHERE id = ?`)
      .bind(p.stored, p.salt, user.id).run();
  }

  const token = newToken();
  return json({ ok: true, token, user: { id: user.id, name: user.name, username: user.username, token } });
}
...
async function createAccount(env, body) {
  ...
  await env.DB.prepare(`
    ...
    VALUES (?,?,?,?,?,?)
  `).bind(id, name, username, p.stored, p.salt, now()).run();

  const token = newToken();
  return json({ ok: true, token, user: { id, name, username, token } });
}

async function changePassword(env, request, body) {
  const auth = String(request.headers.get("Authorization") || "");
  const current = auth.startsWith("Bearer ") ? auth.slice(7) : "";
  const username = clean(body?.username, 120)?.toLowerCase();
...
async function adminResetPassword(env, request, body) {
  ...
  const username = clean(body?.username, 120)?.toLowerCase();
  const newPassword = String(body?.newPassword ?? "");
  if (!username || newPassword.length < 8) return json({ error: "Username and a password of at least 8 characters are required." }, 400);
...
async function trackVisit(request, env) {
  ...
  const existing = await env.DB.prepare(`
    SELECT id FROM visits WHERE visitor_id=? AND path=?
  ...
  const g = geo(request);
  await env.DB.prepare(`
    INSERT INTO visitor_locations (visitor_id,path,country,region,city,timezone,continent,created_at)
    VALUES (?,?,?,?,?,?,?,?)
  `).bind(visitorId,path,g.country,g.region,g.city,g.timezone,g.continent,created).run();
  return json({ ok: true, counted: true });
}

async function stats(env, request) {
  const key = String(request.headers.get("X-Admin-Key") || "");
  if (!env.ADMIN_KEY || !safeEqual(key, env.ADMIN_KEY)) return json({ error: "Unauthorized." }, 401);
  await ensureLocationTable(env);
  ...
  async fetch(request, env) {
    ...
    try {
      ...
      if (url.pathname === "/api/set-password" && request.method === "POST") {
        ...
        const user = await getUser(env, b?.username || "");
        if (!user) return json({ error: "No existing account was found with that username." }, 404);
        if (user.password_salt) return json({ error: "This account already has a password. Please sign in or use account recovery." }, 409);
        const p = String(b?.password || "");
        if (p.length < 8 || p !== String(b?.confirmPassword || "")) return json({ error: "Passwords must match and be at least 8 characters." }, 400);
        const hp = await hashPassword(p);
        await env.DB.prepare(`UPDATE users SET password_hash=?,password_salt=? WHERE id=?`).bind(hp.stored,hp.salt,user.id).run();
        const token = newToken();
        return json({ ok:true,token,user:{id:user.id,name:user.name,username:user.username,token} });
      }
      ...
      if (url.pathname === "/api/reset-password" && request.method === "POST") return json({ error: "Use the recovery code or the admin-protected repair endpoint." }, 400);

      if (url.pathname === "/api/track-event" && request.method === "POST") {
        const p = await request.json().catch(() => ({}));
        if (!p?.eventType) return json({ error: "eventType is required." }, 400);
        await env.DB.prepare(`
          INSERT INTO analytics_events (visitor_id,user_id,event_type,feature,difficulty,question,correct,score,total,metadata,created_at)
          VALUES (?,?,?,?,?,?,?,?,?,?,?)
        `).bind(
          clean(p.visitorId,200),clean(p.userId,200),clean(p.eventType,120),clean(p.feature,120),clean(p.difficulty,120),clean(p.question,500),
          p.correct==null?null:(p.correct?1:0),p.score==null?null:Number(p.score),p.total==null?null:Number(p.total),
