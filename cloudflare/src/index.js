...
async function verifyLegacyPassword(password, stored) {
  ...
  // Existing records were stored as: sha256$SALT$HASH
  // The first '$' is therefore at position 7. Do not change the stored value;
  // verify it and let the caller migrate it to the current format after success.
  if (value.toLowerCase().startsWith('sha256$')) {
    ...
    if (parts.length === 3) {
      ...
      if (/^[a-f0-9]{64}$/i.test(expected)) {
        ...
        if (candidates.some(x => timingSafeEqual(x, expected))) return true;
      }
    }
  }

  // Other legacy layouts used a delimiter between salt and a SHA-256 hash.
  for (const separator of [':', '$', '|']) {
    ...
    if (parts.length === 2) {
      ...
      if (b && /^[a-f0-9]{64}$/i.test(b)) {
        const candidates = [
          await sha256Hex(a + password),
          await sha256Hex(a + ':' + password),
          await sha256Hex(password + a),
          await sha256Hex(password + ':' + a)
        ];
        if (candidates.some(x => timingSafeEqual(x, b))) return true;
      }
      if (a && /^[a-f0-9]{64}$/i.test(a)) {
        const candidates = [
          await sha256Hex(b + password),
          await sha256Hex(b + ':' + password),
          await sha256Hex(password + b),
          await sha256Hex(password + ':' + b)
        ];
        if (candidates.some(x => timingSafeEqual(x, a))) return true;
      }
    }
  }
  ...
  return timingSafeEqual(plain, value);
}

async function verifyPassword(password, user) {
  if (!user || !user.password_hash) return false;
  if (user.password_salt) {
    const hash = await sha256Hex(String(user.password_salt) + ':' + String(password));
    return timingSafeEqual(hash, String(user.password_hash));
  }
